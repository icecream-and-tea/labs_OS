...
#define cmd_size sizeof(struct cmd) //sizeof为单目操作符

struct cmd *ourCmds = NULL;

int listCmds(){
    struct cmd *tmpCmd = ourCmds->nextCmd; //ourcmd作为废的头节点
...
void addNewCmd(	unsigned char *cmd, 
    ...
    */
    //先向系统malloc一个空间,返回空间的首地址
    unsigned long start = malloc(cmd_size);
    //myPrintf(0x7, "newcmd_start = 0x%x\n", start);
    if(start != 0){
        struct cmd newCmd;
        strcpy(newCmd.cmd, cmd);
        strcpy(newCmd.description, description);
        newCmd.func = func;
        newCmd.help_func = help_func;
        //cmd链表头插法
        newCmd.nextCmd = ourCmds->nextCmd;
        ourCmds->nextCmd = (struct cmd*) start;
        //复制到内存中
        memcpy((void*)start, &newCmd, cmd_size);
    }
}
...
void startShell(void){    
    ...
    while(1) {
        ...
        getCmdline(&cmdline[0],100); //从终端获取指令
