...
#define EEB_size (unsigned long)4
...
#define eFPartition_size (unsigned long)12
...
void eFPartitionWalkByAddr(unsigned long EFPHandler){
	...
	*/
	...
	//打印efp
	showeFPartition(p_partition);
	//遍历eeb
	EEB* p_eeb = (EEB*)p_partition->firstFree;
	while(p_eeb != 0){
		showEEB(p_eeb);
		p_eeb = (EEB*)p_eeb->next_start;
	}
}
...
unsigned long eFPartitionTotalSize(unsigned long perSize, unsigned long n){
	...
	*/
	...
	unsigned long totalSize = n * (alignSize + EEB_size) + eFPartition_size;
	return totalSize;
}
