...
int testdP4(int argc, unsigned char **argv){
	unsigned long x,x1,xHandler;
	int i, tsize = 0x100;
	x = kmalloc(tsize); 
	myPrintf(0x7, "x = 0x%x\n", x);
	if (x){
		...
		free(x);
	} else myPrintf(0x7,"KMALLOC FAILED, CAN't TEST dPartition\n");	
}
...
void memTestCaseInit(void){	
	...
	addNewCmd("testdP4\0", testdP4, NULL, "(K)Init a dPatition(size=0x100) [Alloc,Free]* with step = 0x20\0");
